...
默认 `--reduce-mode two-phase`：保持原 `(batch, KV head, block)` task 和动态任务领取方式，
每个 task 将 FP32 输出、LSE 写入自己的暂存槽。所有 block 计算完成后，
再动态领取 `(batch, query head)` 归并任务，使用块 LSE 的稳定 softmax 对块输出加权。
归并只读取块结果，每个输出 query head 由一个归并任务独占，不需要输出 mutex。
不同长度序列仍按各自有效 block 数分配计算任务；归并单独计时，其负载差异也可从逐线程表观察。
额外暂存空间在 Hq=32 时约 33 MiB，Hq=64 时约 65 MiB（当前 B/S/Block 配置），跨调用、跨层复用。
首次扩容会计入初始化，正式测量前的预热会完成扩容。

原带锁路径保留为 `--reduce-mode locked`。重新编译同一版本后，可保存两种模式的报告作对照：
...
两阶段模式的 `merge/sync/writeback/flushes` 为零；写暂存槽的成本包含在 PV 中。
`share_pct` 仅表示块计算阶段各项占比。第二阶段使用单独的 `reduce_tasks` 和 `avg_ms/reduce_task`，
其分母是每个线程实际执行的归并任务数，不混入 block task 计数。
...
两阶段模式每组应有 3 次 attention、24576 个 block task、6144 个 reduce task、0 次 flush。
